...
function Client (options) {
  ...
  if (options.bindDN !== undefined && options.bindCredentials !== undefined) {
    this.on('setup', function (cb) {
      ...
      this.bind(options.bindDN, options.bindCredentials, function (err) {
        if (err) {
          if (socket) {
            socket.destroy()
          }
...
Client.prototype.unbind = function unbind (callback) {
  ...
  if (this._socket && this.connected) {
    ...
    this._dispatchRequest(req, 'unbind', null, callback)
  } else {
...
Client.prototype.starttls = function starttls (options, controls, callback) {
  ...
  if (this._starttls) {
    const err = new Error('STARTTLS already in progress or active')
    if (callback instanceof Promise) {
      callback.reject(err)
      return callback
    }
  ...
  if (this._socket.encrypted) {
    const err = new Error('Socket is already encrypted')
  ...
  this._dispatchRequest(req, [errors.LDAP_SUCCESS], new EventEmitter(), onSend)
  if (callback instanceof Promise) {
    return callback
  }
}
...
Client.prototype.connect = function connect () {
  ...
  retry.on('ready', function (num, _delay) {
    ...
    connectSocket(function (err) {
      if (!err) {
        ...
        self.queue.flush(self._dispatchRequest.bind(self))
  ...
  retry.on('fail', function (err) {
    function emitError (event, connectionError) {
      if (event !== 'error' && connectionError && self.listenerCount(event) === 0) {
        if (typeof connectionError === 'string') {
          connectionError = event + ': ' + connectionError
        } else if (connectionError.message) {
          connectionError.message = event + ': ' + connectionError.message
        }
    ...
    if (err instanceof ConnectionError) {
      emitError('connectTimeout', err)
    } else if (err.code === 'ECONNREFUSED') {
      emitError('connectRefused', err)
    } else {
      self.emit('error', err)
    }

    // maybe socket is connected, but setup failed
...
Client.prototype._dispatchRequest = function _dispatchRequest (req, expect, emitter, callback) {
  ...
  if (!this._socket || !this.connected) {
    if (!this.queue.enqueue(req, expect, emitter, callback)) {
      callback(new ConnectionError('connection unavailable')) // and also request-queue is disabled
    }

    if (this.reconnect) {
      this.connect() // could retry a destroyed or unbound connection
    }
...
Client.prototype._sendRequest = function _sendRequest (req, expect, emitter, callback) {
  ...
  tracker.track(req, function onMessage (msg) { // message.messageID set here as side-effect, btw
    ...
    function dispatch (event, data) {
      ...
      if (event === 'error') {
        // if !emitter, callback should be "virgin" -- it was not used to dispatch emitter upon writing request to socket
        // (emitter should be provided for any request having possibly more than one response message)
        callback(data)
        return
      }
