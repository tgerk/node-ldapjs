  ...
  if (options.tlsOptions) {
    this.on('setup', function (clt, cb) {
      if (!clt._socket.encrypted) {
        clt.starttls(options.tlsOptions, null, cb)
        return
      }
      cb()
    })
...
Client.prototype.abandon = function abandon (messageID, controls, callback) {
  ...
  this._send(req, 'abandon', null, callback)
...
Client.prototype.add = function add (name, entry, controls, callback) {
  ...
  this._send(req, [errors.LDAP_SUCCESS], null, callback)
  if (callback instanceof Promise) {
    return callback
  }
}
...
Client.prototype.bind = function bind (name,
  ...
  this._send(req, [errors.LDAP_SUCCESS], null, callbackWrapper, _bypass)
  if (callback instanceof Promise) {
    return callback
  }
}
...
Client.prototype.compare = function compare (name,
  ...
  this._send(req, CMP_EXPECT, null, callback)
  if (callback instanceof Promise) {
    return callback
  }
}
...
Client.prototype.unbind = function unbind (callback) {
  ...
  if (this._socket && this.connected) {
    ...
    this._send(req, 'unbind', null, callback)
  } else {
...
Client.prototype.starttls = function starttls (options,
  ...
  this._send(req, [errors.LDAP_SUCCESS], new EventEmitter(), onSend, _bypass)
  if (callback instanceof Promise) {
    return callback
  }
}
